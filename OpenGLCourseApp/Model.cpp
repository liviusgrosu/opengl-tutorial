...
void Model::LoadNode(aiNode* node, const aiScene* scene) {
	for (size_t i = 0; i < node->mNumMeshes; i++) {
		// Load all meshes associated with the current node
		LoadMesh(scene->mMeshes[node->mMeshes[i]], scene);
	}

	for (size_t i = 0; i < node->mNumChildren; i++) {
		// Traverse the children node and load their meshes
...
void Model::LoadMesh(aiMesh* mesh, const aiScene* scene) {
	...
	for (size_t i = 0; i < mesh->mNumVertices; i++) {
		...
		if (mesh->mTextureCoords[0]) {
			...
			// Insert a default texture coordinate regardless
			vertices.insert(vertices.end(), { 0.0f, 0.0f });
		}
		// Insert the vertex normal
	...
	for (size_t i = 0; i < mesh->mNumFaces; i++) {
		...
		for (size_t j = 0; j < face.mNumIndices; j++) {
			// Insert the indices off the face
			indices.push_back(face.mIndices[j]);
		}
	}


	// Create new mesh from the imported data
